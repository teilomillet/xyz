...
Second, the directions are robust to SFT budget. With 5× fewer steps and 10× fewer examples, the extracted directions maintain cosine similarity of 0.84 (KromCanon), 0.80 (vanilla), and 0.72 (Canon) with the full-budget directions. KromCanon's multi-stream structure produces the most stable directions; Canon's local convolutions produce the least stable. The main architectural findings do not appear to depend on SFT saturation.
