...
This reveals two distinct thresholds in multi-stream mixing. The first is *geometric*: at around 10% swap probability, doubly stochastic contraction synchronizes per-stream directions (previous section). The second is *functional*: somewhere between 12% and 27%, mixing becomes strong enough to change downstream learning dynamics. Passing the geometric threshold is necessary but not sufficient. Our bias=-2 models pass it (cosines rise to 0.996) but fail the functional threshold (SFT trajectories are identical to frozen). Bias=-1 passes both.
